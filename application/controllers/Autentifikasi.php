...
public function registrasi() 
{
    if ($this->session->userdata('email')) {
        ...
        $this->form_validation->set_rules('email', 'Alamat 
        Email', 'required|trim|valid_email|is_unique[user.
        email]', [
        ...
        'required|trim|min_length[3]|matches
        [password2]', [
            'matches' => 'Password Tidak Sama!!',
            'min_length' => 'Password Terlalu Pendek'
        ]);
        $this->form_validation->set_rules('password2', 
        'Password', 'required|trim|matches
        [password1]');
        ...
        public function logout()
        {
            ...
            $this->session->set_flashdata ('pesan', '<div class="alert alert-danger alert-message" role="alert">Anda Sudah Keluar!!</div>');
                redirect ('autentifikasi');
        }
    }